<button type="button" onclick="openAddConnectionModal('facebook',true,'yes')" class="fb-connection-btn position-relative d-flex flex-column justify-content-center align-items-center bg-transparent connections-connector-wrapper @if(Auth::user() && Auth::user()->company->social_account_facebook->isNotEmpty()) active-connection-wrapper @endif">
    <span>
    ...
    <p class="m-0 primary-text-color connector-primary-name">{{__('Facebook Page ')}}</p>
    
    <i class="position-absolute connection-active-tick @if($get_locale=='ar') connection-active-tick-ar @endif @if(Auth::user() && Auth::user()->company->social_account_facebook->isEmpty()) d-none @endif">
