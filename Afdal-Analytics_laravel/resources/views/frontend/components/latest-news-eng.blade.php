...
<section class="latest-news-section">
    <div class="latest-news-title-wrapper">
        <h2 class="latest-news-title font-48-lh-56-semi-bold">
            {{__('Blogs')}}
        </h2>
        <a class="latest-news-all-button font-20-lh-23-semi-bold" href="/blog">
            {{__('See all')}}
        </a>
    </div>
    @if(!empty($blogs))
                ...
                <li class="latest-news-card">
                    <a href="{{url('/blog/' .$blog->category->seo_url.'/'. $blog->seo_url)}}" style="text-decoration: none">
                        ...
                        <div class="latest-news-card-text">
                            ...
                            <p class="latest-news-card-info font-13-lh-15-semi-bold blog-latest-news-card-text">
...
{{--                                {{ !empty($blog->date) ? date('F j, Y', strtotime($blog->date)) . ' - ' : '' . __(strtoupper($blog->category->name))}}--}}
                            ...
                            <h3 class="font-22-lh-25-semi-bold" style="padding-bottom: 20px;">
                                {{$blog->title}}
                            </h3>
