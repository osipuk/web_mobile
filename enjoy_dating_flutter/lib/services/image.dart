// import 'package:health_doctor_module/constants/colors.dart';
...
Future<File?> pickVideo({bool isGallery = true})async{
  ...
  try{
    XFile? pickedVideo;
    if(isGallery){
      pickedVideo = await picker.pickVideo(source: ImageSource.gallery,maxDuration:Duration(minutes: 1));
  ...
  // pickedVideo.
    ...
    print('the length of the video is,$videoLength');
    print(videoLength);
    // if(videoLength>10485760){
      if(videoLength>1085760){
        showSnackbar( 'Video must be less than 1 mb');
      return null;
    }
