...
export default class Mapprovider extends Component {
  ...
  getData = async () => {
    ...
    let event = { latitude: user_lat, longitude: user_long, latitudeDelta: this.state.latdelta, longitudeDelta: this.state.longdelta }
  ...
  getcurrentlatlogn = async () => {
    ...
    if (result != null && this.props.comingPageName=='edit') {
      let address   =  result.address;
      let latitude  =  result.latitude;
      let longitude =  result.longitude;
      this.setState({ latitude: latitude, longitude: longitude,makermove:1})
    }else if(this.props.comingPageName=='create_job'){
      var address_arr = this.props.address_arr;
      let lat = parseFloat(address_arr.latitude);
      let long = parseFloat(address_arr.longitude);
      if(lat!='' && long!=''){
        this.setState({ latitude: lat, longitude: long,makermove:1})
      }else{
        this.getlatlong();
      }
    }else{
          ...
          <View style={styles.container}>
            <View style={{ width: '100%', alignSelf: 'center', flexDirection: 'row', paddingTop: 10, backgroundColor: '#0088e0' }}>
              ...
              <View style={{ paddingVertical: 15, width: '60%' }}>
                <Text style={{ color: '#ffffff', fontFamily: 'Poppins-Medium', fontSize: 17, textAlign: 'center' }}>Search</Text>
              </View>
              <TouchableOpacity style={{ paddingVertical: 15, width: '20%', alignSelf: 'center' }} onPress={() => { this.DoneButton() }}>
                <View style={{ width: '100%', alignSelf: 'center' }} >
                  <Text style={{ color: '#ffffff', fontFamily: 'Poppins-Medium', fontSize: 13, textAlign: 'center' }}>Done</Text>
                </View>
              </TouchableOpacity>
