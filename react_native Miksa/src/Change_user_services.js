...
import { Text, View, SafeAreaView, StatusBar, Image, TouchableOpacity, TextInput, ScrollView, FlatList, BackHandler, Dimensions } from 'react-native'
...
export default class Change_user_services extends Component {
    ...
    getServicesData = async () => {
        ...
        let url = config.baseURL + 'get_user_services.php?user_id_post=' + user_id;
        consolepro.consolelog('service array', url);
        apifuntion.getApi(url).then((obj) => {
            ...
            if (obj.success == 'true') {
                ...
                for (let i = 0; i < service_arr.length; i++) {
                    if (service_arr[i].status == true) {
                        send_data.push(service_arr[i].service_id);
                    }
                }

                this.setState({ service_arr: service_arr, service_arr1: service_arr, send_services: send_data })
            } else {
                setTimeout(() => {
    ...
    render() {
        return (
            <View style={{ flex: 1, backgroundColor: color1.theme_app }}>
                ...
                <View style={styles.select_servicepage}>
                    <View>
                        <Text style={styles.selec_ro_serice}>
                            {Lang_chg.txt_change_service[config.language]}
                        </Text>
                    </View>
                </View>
                ...
                <View style={styles.homebankbtn, { width: '90%', alignSelf: 'center', marginBottom: 20 }}>
