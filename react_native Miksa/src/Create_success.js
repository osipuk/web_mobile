...
export default class Create_success extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            createtime: this.props.route.params.createtime,
        }
        ...
        return (
            <View style={{ flex: 1, backgroundColor: color1.theme_app }}>
                ...
                <View style={styles.create_success_view}>
                    ...
                    <Text style={styles.jobcreateone}>{Lang_chg.txt_job_suuc[config.language]}</Text>
                    <Text style={styles.jobcreateid}>{Lang_chg.txt_job_job_id[config.language]}: {this.state.job_number}</Text>
                    <Text style={styles.jobcreatedates}>{Lang_chg.txt_job_date[config.language]}: {this.state.createtime}</Text>
                    <View style={{ alignItems: 'center', justifyContent: 'center', width: '75%', alignSelf: 'center', marginBottom: 20, marginTop: 10 }}>
                        <TouchableOpacity style={styles.btn_login} activeOpacity={0.9} onPress={() => { this.props.navigation.navigate('Home_customer') }}>
