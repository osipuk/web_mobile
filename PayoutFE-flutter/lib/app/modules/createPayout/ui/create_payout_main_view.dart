...
import 'package:pay_out/app/modules/createPayout/ui/create_payout_screen.dart';
...
class CreatePayoutMainScreen extends GeneralScreen {
  CreatePayoutViewModel? model;

  CreatePayoutScreen? payOutScreen;

  CreatePayoutMainScreen({
  ...
  Widget builderView(
      BuildContext context, CreatePayoutViewModel model, RouteData routeData) {
    this.model = model;
    return Stack(
      children: [
  ...
  Widget contentPayOut(BuildContext context) {
    ...
    payOutScreen = CreatePayoutScreen(
      onBackCallback,
  ...
  void updateStepView(int currentStep, List<CreatePayOutSteps> totalSteps) {
